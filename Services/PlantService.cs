...
namespace PlantApp.Services
{
    public class PlantService
    {
        ...
        public async Task<List<Plant>> GetPlantsAsync()
        {
            ...
            var count = await _db.Table<Plant>().CountAsync();// 2. Проверка состояния БД
            if (count == 0)                                     // 3. Сценарий первого запуска
            {
                var plants = new List<Plant>              // 4. Сидинг в ДИСК
                {
                    new Plant
                    {
                        NamePlant = "Monstera",
                ...
                await _db.InsertAllAsync(plants);
            }
            return await _db.Table<Plant>().ToListAsync();   // 5. Гарантированный результат
        }


        public async Task InitAsync()  
        {
            await _db.CreateTableAsync<Plant>();
        }
